...
class DynamicContractTests: XCTestCase {
    ...
    func testDeployingContract() {
        ...
        do {
            let contract = try web3.eth.Contract(abi: data)
            
            XCTAssertNotNil(contract.constructor, "constructor should be parsed when provided")
            ...
            guard let invocation = contract.deploy(byteCode: byteCode, parameters: BigUInt(5)) else {
                return XCTFail("Could not invoke")
            }
            
            XCTAssertEqual(invocation.parameters.count, 1)
            ...
            XCTAssertEqual(generatedHexString, rawByteCode + "5".paddingLeft(toLength: 64, withPad: "0"))
            
            let failureExpectation = expectation(description: "Non-payable deploy should fail when including a value")
            ...
            let successExpectation = expectation(description: "Deploy should succeed")
            
            invocation.send(from: .testAddress, gas: 15000, gasPrice: nil) { (hash, error) in
                XCTAssertNil(error)
                XCTAssertEqual(hash, try? EthereumData(ethereumValue: "0x0e670ec64341771606e55d6b4ca35a1a6b75ee3d5145a99d05921026d1527331"), "Transaction hash should come back correctly")
